...
export default class FeedItem extends React.Component {
  ...
  render () {
    ...
    switch(data.type) {
      case "statusUpdate":
      contents = (
        <StatusUpdate key={data._id} author={data.contents.author}
          postDate={data.contents.postDate} location={data.contents.location}>
    ...
    return (
      <div className="fb-status-update panel panel-default">
        ...
        <div className="panel-footer">
          <div className="row">
            <div className="col-md-12">
              <a href="#">{data.likeCounter.lenght} people</a> like this
            </div>
          </div>
          <hr />
            ...
            data.comments.map((comment, i) => {
              return (
                <Comment key={i} author={comment.author} postDate={comment.postDate}
                  postNum={i} itemId={this.state._id} likeCounter={comment.likeCounter}>
                   {comment.contents}
                </Comment>
              );
            })
